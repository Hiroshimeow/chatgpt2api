    ...
    <div className="grid h-[calc(100vh-132px)] min-h-[640px] items-stretch gap-0 overflow-hidden rounded-xl border border-stone-200 bg-white/70 xl:grid-cols-2">
      <section className="space-y-4 overflow-y-auto border-b border-stone-200 p-4 xl:border-r xl:border-b-0">
          ...
          <div className="flex items-start gap-2 rounded-xl border border-sky-200 bg-sky-50 px-3 py-2 text-xs leading-5 text-sky-800">
            ...
            <span>如果注册日志出现 Cloudflare 拦截，可在Cài đặt页Bật FlareSolverr 清障；相关 Docker 容器需要先启动。</span>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="space-y-2">
              ...
              <Select value={config.mode || "total"} onValueChange={(value) => setMode(value as "total" | "quota" | "available")} disabled={config.enabled}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="quota">号池Quota còn lại</SelectItem>
            ...
            <div className="space-y-2">
              <label className="text-sm text-stone-700">目标Quota còn lại</label>
            ...
            <div className="space-y-2">
              <label className="text-sm text-stone-700">Số tài khoản khả dụng mục tiêu</label>
            ...
            <div className="space-y-2">
              <label className="text-sm text-stone-700">Khoảng kiểm tra（秒）</label>
          ...
          <div className="space-y-3 border-t border-stone-200 pt-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-stone-500">Có thể cấu hình nhiều provider, xoay vòng theo thứ tự bật.</p>
              </div>
                      ...
                      {type === "cloudmail_gen" || type === "cloudflare_temp_email" || type === "moemail" || type === "inbucket" || type === "yyds_mail" || type === "ddg_mail" ? (
                        <>
                          ...
                          {type === "cloudmail_gen" ? (
                            <>
                              <div className="space-y-2">
                                <label className="text-sm text-stone-700">Email admin</label>
                              ...
                              <div className="space-y-2">
                                <label className="text-sm text-stone-700">Mật khẩu admin</label>
                      ...
                      {type === "ddg_mail" ? (
                        ...
                        <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                          ...
                          <ol className="list-decimal list-inside space-y-0.5">
                            <li>先在 <a href="https://duckduckgo.com/email/" target="_blank" className="underline">DuckDuckGo Email Protection</a> 登录并Cài đặt转发目标为 CF 收件箱地址</li>
                    ...
                    {type === "outlook_token" ? (() => {
                      ...
                      return (
                        <div className="space-y-2">
                          ...
                          <Textarea value={String(provider.mailboxes || "")} onChange={(event) => updateProvider(index, { mailboxes: event.target.value })} placeholder={"每行一个邮箱，格式：\n邮箱----密码----client_id----refresh_token\n（出于安全，已保存的密码/refresh_token 不会回显；此处仅用于Thêm mới或覆盖）"} className="min-h-32 rounded-xl border-stone-200 bg-white font-mono text-xs" disabled={config.enabled} />
                          ...
                          <p className="text-xs text-stone-500">每个邮箱仅成功注册一次（状态记录在 data/outlook_token_used.json）。Thất bại的邮箱会被标记原因，可用上方按钮释放后重试。</p>
                        </div>
                      );
                    })() : null}
